...
import { LocationService, RetailerLocation } from '../location.service';
import { ResultCode } from 'src/core/constant/AppEnums';
import { appUrl } from '../../app-url';
import { ActivatedRoute, Params } from '@angular/router';
import { ResultModel } from 'src/core/models/http.model';
import { AppConsts } from 'src/core/constant/AppConsts'
import { GeoService, Country, Province, District, Ward } from 'src/aritnt/administrator/geographical/geo.service';
...
export class LocationDetailComponent extends AppBaseComponent {
  ...
  private isDescriptionValid: boolean = false;
  ...
  constructor(
    injector: Injector,
    private collSvc: LocationService,
  ...
  private return() {
    console.log('return');
    this.router.navigate([appUrl.locationList]);
  }
  ...
  private save() {
    ...
    //this.location.retailerId = this.authenticSvc.getSession().

    if(this.type == "update")
    {
      this.collSvc.update(this.location).subscribe((result: ResultModel<any>) => {
        if(result.result == ResultCode.Success)
        {
          //alert
        ...
        this.collSvc.add(this.location).subscribe((result: ResultModel<any>) => {
          if(result.result == ResultCode.Success)
          {
            //alert
  ...
  private checkValid() : boolean{
    return this.isNameValid && this.isAddressWardValid &&
           ...
           this.isContactEmailValid  && this.isDescriptionValid;
  }
